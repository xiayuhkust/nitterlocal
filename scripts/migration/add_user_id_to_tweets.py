#!/usr/bin/env python3
"""
Migration script to add user_id, is_reply, reply_to, and conversation_id columns to the tweets table.
"""
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Add user_id, is_reply, reply_to, and conversation_id columns to the tweets table')
    parser.add_argument('--dry-run', action='store_true', help='Dry run - do not make any changes')
    
    args = parser.parse_args()
    
    print(f"Migration script to add columns to tweets table in {SQLITE_DB_PATH}")
    ...
    missing_columns = check_columns_exist(conn, 'tweets', columns_with_types.keys())
    
    if not missing_columns:
        print("All columns already exist in the tweets table")
        conn.close()
        return
    ...
    if args.dry_run:
        ...
        add_columns(conn, 'tweets', missing_columns_with_types)
