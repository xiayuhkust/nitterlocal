#!/bin/bash
# Script to set up crontab entries for MySQL synchronization with detailed logging
...
# Add the new entry for sync_to_mysql.py which includes tweet synchronization
echo "# Every 15 minutes, synchronize to MySQL with detailed logging" >> "$TEMP_CRONTAB"
echo "*/15 * * * * cd $NITTER_DIR && python3 scripts/sync/sync_to_mysql.py --since-days 1 >> data/sync_cron.log 2>&1" >> "$TEMP_CRONTAB"
...
echo "MySQL synchronization crontab entry installed successfully."
echo "The following entry was added to your crontab:"
echo "*/15 * * * * cd $NITTER_DIR && python3 scripts/sync/sync_to_mysql.py --since-days 1 >> data/sync_cron.log 2>&1"
