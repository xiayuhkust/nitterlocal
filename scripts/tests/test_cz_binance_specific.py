#!/usr/bin/env python3
"""
Script to specifically test the processing of cz_binance data.
"""
...
def create_cz_binance_excel(output_path):
    """Create a test Excel file with only cz_binance data"""
    logging.info(f"Creating test Excel with cz_binance data at {output_path}")
    ...
    try:
        ...
        logging.info(f"Created test Excel with cz_binance data at {output_path}")
        return True
    except Exception as e:
        logging.error(f"Error creating test Excel with cz_binance data: {str(e)}")
        return False

def process_cz_binance_excel(excel_path):
    """Process the cz_binance Excel file and show the results"""
    logging.info(f"Processing cz_binance Excel file: {excel_path}")
    ...
    try:
        ...
        logging.error(f"Error processing cz_binance Excel: {str(e)}")
        return False
...
def main():
    ...
    create_cz_binance_excel(excel_path)
    
    # Process the cz_binance Excel file
    process_cz_binance_excel(excel_path)
