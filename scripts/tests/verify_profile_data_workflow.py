#!/usr/bin/env python3
"""
Test script to verify the integrated Excel processing workflow with profile data.
"""
...
def process_excel_with_profile(excel_path, db_path):
    """Process the Excel file with profile data extraction"""
    logging.info(f"Processing Excel file with profile data: {excel_path}")
...
def verify_profile_data(db_path):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM url_tracking WHERE screen_name = ?", ('cz_binance',))
        ...
        if url_record:
            ...
            cursor.execute("SELECT * FROM kol_character WHERE kol_screen_name = ?", ('cz_binance',))
...
def main():
    ...
    try:
        ...
        # Create url_tracking table with all necessary columns
        cursor.execute('''
        ...
        # Process the Excel file with profile data
        if not process_excel_with_profile(excel_path, db_path):
            logging.error("Failed to process Excel file with profile data")
            return False
        ...
        logging.info("Profile data workflow test completed successfully")
        ...
        logging.error(f"Error in profile data workflow test: {str(e)}")
        return False
