#!/usr/bin/env python3
"""
Comprehensive demonstration script for Excel processing with both twitter.com and x.com URLs.
This script:
...
4. Displays the resulting database entries with all fields
"""
...
def display_database_entries(db_path, expected_screen_names):
    """Display the database entries to verify URL normalization and data storage"""
    ...
    # Check for each expected screen name
    for screen_name in expected_screen_names:
        logging.info(f"\n=== Data for {screen_name} ===")
        
        # Check url_tracking table
        if 'screen_name' in url_tracking_columns:
            cursor.execute("SELECT * FROM url_tracking WHERE screen_name = ? COLLATE NOCASE", (screen_name,))
        else:
            cursor.execute("SELECT * FROM url_tracking WHERE url LIKE ? COLLATE NOCASE", (f"%/{screen_name}%",))
        
        url_tracking_rows = cursor.fetchall()
        
        if url_tracking_rows:
            logging.info(f"=== Data for {screen_name} in url_tracking table ===")
            for i, col in enumerate(url_tracking_columns):
                logging.info(f"{col}: {url_tracking_rows[0][i]}")
            
            # Get the url_tracking_id for this screen name
            url_tracking_id = None
            for i, col in enumerate(url_tracking_columns):
                if col == 'id':
                    url_tracking_id = url_tracking_rows[0][i]
                    break
            ...
            if url_tracking_id is not None:
                ...
                kol_character_rows = cursor.fetchall()
                
                if kol_character_rows:
                    logging.info(f"\n=== Data for {screen_name} in kol_character table ===")
                    for i, col in enumerate(kol_character_columns):
                        logging.info(f"{col}: {kol_character_rows[0][i]}")
                else:
                    logging.warning(f"No data found for {screen_name} in kol_character table")
        else:
            logging.warning(f"No data found for {screen_name} in url_tracking table")
    
    conn.close()
    
    return x_com_count == 0 and twitter_com_count > 0

def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Demonstrate Excel processing with both twitter.com and x.com URLs")
    parser.add_argument("--rows", type=int, default=3, help="Number of rows to include in the test Excel file")
    ...
    expected_screen_names = create_test_excel(excel_path, args.rows)
    ...
    if display_database_entries(db_path, expected_screen_names):
