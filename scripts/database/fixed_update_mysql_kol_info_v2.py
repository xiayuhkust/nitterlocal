...
Version 2 with fixed cursor handling to resolve "Unread result found" errors.
"""
...
def insert_or_update_kol_info(mysql_conn, kol_info):
    ...
    try:
        ...
        logging.error("Error inserting or updating record for kol_id {}: {}".format(kol_info['kol_id'], str(e)))
        if mysql_conn:
            mysql_conn.rollback()
        raise
    finally:
