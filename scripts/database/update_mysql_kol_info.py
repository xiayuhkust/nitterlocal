...
This script retrieves profile information from Twitter accounts and maps them to the kol_info table schema.
"""
    ...
    if profile.get('joined'):
        try:
            ...
            if match:
                ...
                logging.warning(f"Could not parse date: {iso_date}")
        except Exception as e:
...
def insert_or_update_kol_info(mysql_conn, kol_info):
    ...
    if result:
        ...
        logging.info(f"Updated record for kol_id: {kol_info['kol_id']}")
    else:
        # Insert new record
        insert_query = """
        ...
        logging.info(f"Inserted new record for kol_id: {kol_info['kol_id']}")
...
def get_urls_from_sqlite(limit=None):
    ...
    query = "SELECT url, user_id, type, subtype FROM url_tracking WHERE user_id IS NOT NULL"
...
def main():
    ...
    logging.info(f"Got {len(urls)} URLs from SQLite")
    ...
    for url_data in urls:
        ...
        if not args.test:
            ...
            logging.info(f"Test mode - would insert or update record for kol_id: {kol_info['kol_id']}")
        
        processed_count += 1
    
    logging.info(f"Processed {processed_count} profiles")
