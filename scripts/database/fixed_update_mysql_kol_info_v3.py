...
Python 3.6 compatible version for server deployment with improved error handling.
Version 3 with completely rewritten cursor handling to resolve "Unread result found" errors.
"""
...
def get_mysql_connection():
    ...
    try:
        ...
        conn = mysql.connector.connect(
            ...
            autocommit=False,
            consume_results=True  # Important: consume results automatically
        )
        return conn
    except ImportError:
...
def get_basic_profile_info(handle):
    ...
    # Generate a numeric ID from the handle
    import hashlib
    ...
    profile = {
        "profile": {
            "userId": str(numeric_id),  # Using a numeric ID generated from the handle
            "name": handle,
            "username": handle,
            "biography": None,
            "followersCount": None,
...
def check_record_exists(mysql_conn, kol_id):
    """Check if a record exists in the kol_info table"""
    cursor = None
    try:
...
def update_kol_info(mysql_conn, kol_info):
    """Update an existing record in the kol_info table"""
    cursor = None
    try:
        cursor = mysql_conn.cursor()
        update_query = """
        ...
        logging.error("Error updating record for kol_id {}: {}".format(kol_info['kol_id'], str(e)))
        mysql_conn.rollback()
        return False
    finally:
        if cursor:
...
def insert_kol_info(mysql_conn, kol_info):
    """Insert a new record into the kol_info table"""
    cursor = None
    try:
        cursor = mysql_conn.cursor()
        insert_query = """
        ...
        logging.error("Error inserting record for kol_id {}: {}".format(kol_info['kol_id'], str(e)))
        mysql_conn.rollback()
        return False
    finally:
        if cursor:
...
def insert_or_update_kol_info(mysql_conn, kol_info):
    ...
    try:
        ...
        exists = check_record_exists(mysql_conn, kol_info['kol_id'])
        
        # Update or insert based on existence
        if exists:
            return update_kol_info(mysql_conn, kol_info)
        else:
            return insert_kol_info(mysql_conn, kol_info)
    except Exception as e:
        logging.error("Error in insert_or_update_kol_info for kol_id {}: {}".format(kol_info['kol_id'], str(e)))
        return False
...
def main():
    ...
    print("Starting MySQL update script (Node.js independent version)")
    logging.info("Starting MySQL update script (Node.js independent version)")
    ...
    try:
        for url_data in urls:
            ...
            if not args.test:
                if insert_or_update_kol_info(mysql_conn, kol_info):
                    success_count += 1
            else:
            ...
            # Break after first URL in test mode
            if args.test and processed_count >= 1:
                logging.info("Test mode - processed one URL successfully, stopping")
                break
    except Exception as e:
        logging.error("Error processing URLs: {}".format(str(e)))
    finally:
    ...
    logging.info("Processed {} profiles, {} successful".format(processed_count, success_count))
